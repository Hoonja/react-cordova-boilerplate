...
class VideoPhone3 extends Component {
  componentDidMount() {
  ...
  onRtcReadyToCall(session) {
    console.log('[VideoPhone] onRtcReadyToCall', session, this.rtc);
  ...
  connect = (serverUrl, roomId, actorId, actorName) => {
    console.log('[VideoPhone] connect', serverUrl, roomId, actorId, actorName);
    this.serverUrl = service.getUrl();
    this.roomId = '7291_7293';//roomId;
    this.actorId = 7292;//actorId;
